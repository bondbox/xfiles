...
class TestScanner(TestCase):
    ...
    def setUpClass(cls):
        cls.scanner = Scanner.load(
            paths=[join("xkits_file")],
            exclude=[join("xkits_file", "unittest")],
            handler=handler)
    ...
    def test_load(self):
        self.assertIsInstance(Scanner.load(paths=[join("scanloader")], handler=handler), Scanner)  # noqa:E501
    ...
    def test_iter(self):
        for object in self.scanner:
            ...
            if object.isfile and not object.issym:
                self.assertIsInstance(object.hash(md5())[0], str)
                self.assertIsInstance(object.hash(sha1())[0], str)
                self.assertIsInstance(object.hash(sha256())[0], str)
    ...
    def test_add_dir_object(self):
        object = Scanner.Object(join("xkits_file", "unittest"))
